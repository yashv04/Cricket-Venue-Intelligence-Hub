...
def predict_batters(df, venue, opponent):
    ...
    top_batters = merged.sort_values('predicted_score', ascending=False)[
        ['batter', 'avg_runs', 'recent_avg', 'vs_team_avg', 'predicted_score', 'total_runs', 'strike_rate']
    ]
        ...
        with tab2:
            ...
            if not venue_toss.empty:
                ...
                if not venue_data.empty:
                    ...
                    elif bowl_win_pct > 55:
                        strategy_tips.append(f"**Strongly prefer bowling first** ({bowl_win_pct:.1f}% win rate)")
                    else:
                        strategy_tips.append("**Toss advantage is minimal** - consider team strengths instead")
                    
                    # Check phase stats
                    phase_summary, _ = analyze_phases(df[df['venue'] == strategy_venue])
                    
                    if not phase_summary.empty:
                        powerplay_data = phase_summary[phase_summary['phase'] == 'Powerplay']
                        death_data = phase_summary[phase_summary['phase'] == 'Death Overs']
                        
                        if not powerplay_data.empty and not death_data.empty:
                            pp_rr = powerplay_data['run_rate'].iloc[0]
                            death_rr = death_data['run_rate'].iloc[0]
                            
                            if pp_rr > 8.5:
                                strategy_tips.append(f"**Aggressive powerplay** recommended (PP Run Rate: {pp_rr:.2f})")
                            
                            if death_rr > 11:
                                strategy_tips.append(f"**Death overs crucial** - assign best bowlers (Death RR: {death_rr:.2f})")
                                
                            # Calculate if venue is spin or pace friendly
                            try:
                                stadium_data = stadium_reports[stadium_reports['venue'] == strategy_venue]
                                if not stadium_data.empty:
                                    spin_assist = stadium_data['Spin Assistance'].iloc[0]
                                    seam_move = stadium_data['Seam Movement'].iloc[0]
                                    
                                    if spin_assist in ['High', 'Very High']:
                                        strategy_tips.append(f"**Spin-friendly venue** - consider extra spinners")
                                    elif seam_move in ['High', 'Very High']:
                                        strategy_tips.append(f"**Seam-friendly venue** - prioritize pace bowlers")
                            except:
                                pass
                    
                    # Display all strategy tips
                    for tip in strategy_tips:
                        st.markdown(tip)
                    
                    # Add visual target score guidance
                    st.subheader("Target Score Guidance")
                    
                    par_score = avg_1st
                    winning_score = par_score * 1.1
                    
                    fig = go.Figure(go.Indicator(
                        mode = "gauge+number+delta",
                        value = winning_score,
                        domain = {'x': [0, 1], 'y': [0, 1]},
                        title = {'text': "Recommended Target Score", 'font': {'size': 24}},
                        delta = {'reference': par_score, 'increasing': {'color': "green"}},
                        gauge = {
                            'axis': {'range': [None, winning_score * 1.2], 'tickwidth': 1},
                            'bar': {'color': "darkgreen"},
                            'bgcolor': "white",
                            'borderwidth': 2,
                            'bordercolor': "gray",
                            'steps': [
                                {'range': [0, par_score * 0.9], 'color': 'red'},
                                {'range': [par_score * 0.9, par_score], 'color': 'orange'},
                                {'range': [par_score, winning_score], 'color': 'lightgreen'},
                            ],
                            'threshold': {
                    ...
                    st.warning("Not enough data available for strategy analysis at this venue.")
            else:
                st.warning("No toss impact data available for this venue.")
else:
    st.error("Data files not found. Please upload the required CSV files.")

# Footer
st.markdown("---")
st.markdown("Cricket Venue Intelligence Hub | Developed by Yash Vardhan")
